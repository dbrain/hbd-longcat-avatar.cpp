...
inline svae::Mesh m4_decode_mesh(const std::vector<int32_t>& coords_in,
    ...
    if (out_coords1024) *out_coords1024 = coords;   // A1: grid-`resolution` occupancy for the marching-tet remesh
    return svae::flexible_dual_grid_to_mesh(coords.data(), N, dual.data(), inter.data(), qlerp.data(), resolution, close_surface);
}
