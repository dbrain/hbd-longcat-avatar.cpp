...
if { [ "$BASE" = "geometry_e2e" ] || [ "$BASE" = "pixal3d" ]; } && [ "$MODE" = "cuda" ]; then
  ...
  echo ">> CUDA build geometry_e2e (ggml-cuda graphs + spike conv)"
  ...
  "$TOOL/bin/g++" $COMMON -fopenmp -DM1_USE_CUDA -DM3A_USE_CUDA $INC -I"$TOOL/include" \
    "$HERE/$SRC" "$HERE/sparse_subm_conv.o" -o "$HERE/$BIN" $LIBS $CUDALIBS -lm \
...
if { [ "$BASE" = "m3a_upsample" ] || [ "$BASE" = "m4_mesh" ] || [ "$BASE" = "m6_tex_decode_test" ]; } && [ "$MODE" = "cuda" ]; then
  TOOL=/mnt/hdd/3d/avatar-shootout/toolchain
  SPIKE="$HERE/../../sparse_spike"
