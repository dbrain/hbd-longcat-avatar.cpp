...
if { [ "$BASE" = "geometry_e2e" ] || [ "$BASE" = "pixal3d" ] || [ "$BASE" = "image_to_rig" ] || [ "$BASE" = "texture_mesh_native" ]; } && [ "$MODE" = "cuda" ]; then
  ...
  # UV rasterisation is independent of ggml inference.  Compile it as a
  # small CUDA companion so the packed-atlas surface buffers no longer have
  # to be generated by the CPU for CUDA texture runs.
  "$TOOL/bin/nvcc" -O2 -std=c++17 -arch=sm_86 -ccbin "$HOSTCXX" \
    -c "$HERE/tex_atlas_cuda.cu" -o "$HERE/tex_atlas_cuda.o"
  ...
  if [ "$BASE" = "image_to_rig" ]; then
    ...
    # image_to_rig links the rig path (rig_pipeline.hpp), whose batched beam
    # (rig_beam_generate_batched.hpp) calls the Philox exponential sampler
    # defined in rig_philox_race.cu.  skintokens_e2e already compiles+links this
    # TU; the shared geometry block did not, which broke image_to_rig's link.
    # curand here is device-only (header kernels), so no extra host lib is needed.
    "$TOOL/bin/nvcc" -O2 -std=c++17 -arch=sm_86 -ccbin "$HOSTCXX" \
...
# narrow_band_dc_probe: native diagnostic for Pixal3D's Python narrow-band UDF dual contouring.
# It deliberately has no Torch/Python/CuMesh runtime dependency: a small scalar CUDA BVH is embedded
# in the probe so a clean-clay candidate can be tested on the 3060 before wiring it into image_to_rig.
if [ "$BASE" = "narrow_band_dc_probe" ] && [ "$MODE" = "cuda" ]; then
  TOOL="${PIXAL3D_CUDA_ROOT:-/mnt/hdd/3d/avatar-shootout/toolchain-cuda13.3}"
  HOSTCXX="${PIXAL3D_HOST_CXX:-/usr/bin/g++-15}"
  TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
...
# philox_exponential_test: isolated CUDA/curand fixture for the PyTorch 2.7
# CUDA exponential race used by multinomial(..., replacement=false).  This is
# deliberately standalone: it validates generator lane semantics before the
# native batched sampled-beam implementation consumes the stream.
if [ "$BASE" = "philox_exponential_test" ] && [ "$MODE" = "cuda" ]; then
  TOOL="${PIXAL3D_CUDA_ROOT:-/mnt/hdd/3d/avatar-shootout/toolchain-cuda13.3}"
  HOSTCXX="${PIXAL3D_HOST_CXX:-/usr/bin/g++-15}"
  TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
  echo ">> CUDA build philox_exponential_test (curand Philox/exponential parity fixture)"
  "$TOOL/bin/nvcc" -O2 -std=c++17 -arch=sm_86 -ccbin "$HOSTCXX" -I"$TOOL/include" \
    "$HERE/$BASE.cu" -o "$HERE/$BIN" -L"$TOOLLIB" -lcudart -lcurand -L/usr/lib -lcuda -lm \
    -Xlinker -rpath -Xlinker "$TOOLLIB" -Xlinker -rpath -Xlinker /usr/lib
  echo ">> built $BIN"
  exit 0
fi

# philox_race_selection_test: fixed-score full selection parity for the native
# sampled-beam primitive. Links only the Philox race helper, never the decoder.
if [ "$BASE" = "philox_race_selection_test" ] && [ "$MODE" = "cuda" ]; then
  TOOL="${PIXAL3D_CUDA_ROOT:-/mnt/hdd/3d/avatar-shootout/toolchain-cuda13.3}"
  HOSTCXX="${PIXAL3D_HOST_CXX:-/usr/bin/g++-15}"
  TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
  echo ">> CUDA build philox_race_selection_test (fixed-score torch multinomial parity)"
  "$TOOL/bin/nvcc" -O2 -std=c++17 -arch=sm_86 -ccbin "$HOSTCXX" -I"$TOOL/include" \
    "$HERE/$BASE.cu" "$HERE/rig_philox_race.cu" -o "$HERE/$BIN" -L"$TOOLLIB" -lcudart -lcurand -L/usr/lib -lcuda -lm \
    -Xlinker -rpath -Xlinker "$TOOLLIB" -Xlinker -rpath -Xlinker /usr/lib
  echo ">> built $BIN"
  exit 0
fi

# tex_atlas_cuda_test: isolated CUDA UV raster parity fixture. It exercises the
# same object linked by texture_mesh_native without requiring model weights.
if [ "$BASE" = "tex_atlas_cuda_test" ] && [ "$MODE" = "cuda" ]; then
  TOOL="${PIXAL3D_CUDA_ROOT:-/mnt/hdd/3d/avatar-shootout/toolchain-cuda13.3}"
  HOSTCXX="${PIXAL3D_HOST_CXX:-/usr/bin/g++-15}"
  TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
  echo ">> CUDA build tex_atlas_cuda_test"
  "$TOOL/bin/nvcc" -O2 -std=c++17 -arch=sm_86 -ccbin "$HOSTCXX" -c "$HERE/tex_atlas_cuda.cu" -o "$HERE/tex_atlas_cuda.o"
  "$HOSTCXX" $COMMON -I"$TOOL/include" "$HERE/$SRC" "$HERE/tex_atlas_cuda.o" -o "$HERE/$BIN" \
    -L"$TOOLLIB" -lcudart -L/usr/lib -lcuda -lm -Wl,-rpath,"$TOOLLIB" -Wl,-rpath,/usr/lib
  echo ">> built $BIN"
  exit 0
fi
...
# rig_harmonic_weights: an explicit generic skinning fallback.  It solves
# libigl's topology-aware harmonic coordinates on a QEM-reduced clean surface,
# then emits NPY weights for the existing textured-mesh transfer/export path.
# libigl + Eigen are already vendored by ManifoldPlus; no Python/Torch/CUDA.
if [ "$BASE" = "rig_harmonic_weights" ]; then
  CXX="${CXX:-/usr/bin/g++}"
  TP="$HERE/../../../thirdparty"
  IGL="$TP/ManifoldPlus/3rd_party/libigl/include"
  EIGEN="$TP/ManifoldPlus/3rd_party/eigen"
  echo ">> build rig_harmonic_weights (libigl surface biharmonic + meshopt QEM, CPU)"
  # libigl's vendored compatibility implementation still uses Eigen's
  # deprecated DynamicSparseMatrix internally; silence that third-party-only
  # warning while retaining warnings for this tool's own code.
  "$CXX" -O2 -std=c++17 -fopenmp -Wno-deprecated-declarations -I"$IGL" -I"$EIGEN" "$HERE/$SRC" \
    "$TP/meshoptimizer/simplifier.cpp" -o "$HERE/$BIN" -lm -lpthread
  echo ">> built $BIN"
  exit 0
fi

# rig_clean_surface_skin: transfers a learned sampled field onto the clean
# watertight pre-texture surface, smooths there, then exports NPYs for the
# normal textured GLB combiner.  This deliberately avoids UV-chart smoothing.
if [ "$BASE" = "rig_clean_surface_skin" ]; then
  CXX="${CXX:-/usr/bin/g++}"
  TP="$HERE/../../../thirdparty"
  echo ">> build rig_clean_surface_skin (clean topology skin refinement + meshopt QEM, CPU)"
  "$CXX" -O2 -std=c++17 -fopenmp "$HERE/$SRC" "$TP/meshoptimizer/simplifier.cpp" \
    -o "$HERE/$BIN" -lm -lpthread
  echo ">> built $BIN"
  exit 0
fi

# p3sam_mask_view_native: native counterpart to the diagnostic Python mask
# helper. It only prepares a geometry inference view from explicit P3-SAM
# labels; it never creates skeletons or weights.
if [ "$BASE" = "p3sam_mask_view_native" ]; then
  CXX="${CXX:-/usr/bin/g++}"
  echo ">> build p3sam_mask_view_native (GLB + NPY label mask, no Python)"
  "$CXX" -O2 -std=c++17 "$HERE/$SRC" -o "$HERE/$BIN" -lm
  echo ">> built $BIN"
  exit 0
fi
...
# mesh_exact_clean_glb: exact-only mesh weld/degenerate cleanup used directly
# after UltraShape extraction. Header-only GLB reader/writer; no CUDA/ggml.
if [ "$BASE" = "mesh_exact_clean_glb" ]; then
  CXX="${CXX:-/usr/bin/g++}"
  echo ">> build mesh_exact_clean_glb (exact-position weld + degenerate cleanup, no CUDA)"
  "$CXX" -O2 -std=c++17 "$HERE/$SRC" -o "$HERE/$BIN" -lm
  echo ">> built $BIN"
  exit 0
fi
...
# atlas_recovery_audit: final GLB/baseColor texture sampled against the original sparse PBR volume.
# It is a CPU-only delivery verifier and intentionally has no xatlas, CUDA, or Python dependency.
if [ "$BASE" = "atlas_recovery_audit" ]; then
  CXX="${CXX:-/usr/bin/g++}"
  echo ">> build atlas_recovery_audit (GLB UV + PNG recovery against sparse PBR, CPU only)"
  "$CXX" -O2 -std=c++17 -fopenmp "$HERE/$SRC" -o "$HERE/$BIN" -lm
  echo ">> built $BIN"
  exit 0
fi
...
if [ "$BASE" = "tex_bake_test" ] || [ "$BASE" = "tex_bake_trellis" ] || [ "$BASE" = "texture_rebake_native" ] || [ "$BASE" = "remesh_test" ] || [ "$BASE" = "tex_bake_dump" ] || [ "$BASE" = "tex_reproject" ] || [ "$BASE" = "texproj_probe" ] || [ "$BASE" = "native_atlas_project" ] || [ "$BASE" = "cumesh_clean_glb" ] || [ "$BASE" = "dc_simplify_glb" ]; then
  if [ "$MODE" = "cuda" ] && { [ "$BASE" = "tex_reproject" ] || [ "$BASE" = "cumesh_clean_glb" ] || [ "$BASE" = "dc_simplify_glb" ] || [ "$BASE" = "texture_rebake_native" ]; }; then
    TOOL="${PIXAL3D_CUDA_ROOT:-/mnt/hdd/3d/avatar-shootout/toolchain-cuda13.3}"
  # Host compiler for nvcc. NOT the system gcc: nvcc 13.3 hard-errors above gcc 15 and this box
  # ships gcc 16. gcc15 is the stock Arch package (side-by-side, does not touch the default).
  HOSTCXX="${PIXAL3D_HOST_CXX:-/usr/bin/g++-15}"
  TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
    ...
    "$HOSTCXX" $COMMON -fopenmp -DTEXATLAS_NATIVE_CUMESH -DTEXATLAS_CUDA_RASTER -I"$TOOL/include" -I"$CUMESH/src" \
      "$HERE/$SRC" "$HERE/native_cumesh_bridge.cpp" "$TP/xatlas.cpp" "$TP/meshoptimizer/simplifier.cpp" \
      "$HERE/tex_atlas_cuda.o" $CUMESH_OBJS -o "$HERE/$BIN" -L"$TOOLLIB" -lcudart -L/usr/lib -lcuda -lm -lpthread \
      -Wl,-rpath,"$TOOLLIB" -Wl,-rpath,/usr/lib
    echo ">> built $BIN"
    exit 0
  fi
...
if [ "$BASE" = "rig_skin_decoder_test" ]; then
  ...
  HOSTCXX="${PIXAL3D_HOST_CXX:-/usr/bin/g++-15}"
    TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
    LIBS="-L$BUILD/src -L$BUILD/src/ggml-cuda -lggml -lggml-base -lggml-cpu -lggml-cuda"
    CUDALIBS="-L$TOOLLIB -lcudart -lcublas -lcublasLt -L/usr/lib -lcuda"
...
if [ "$BASE" = "skintokens_e2e" ]; then
  ...
  TOOLLIB="$TOOL/lib64"; [ -d "$TOOLLIB" ] || TOOLLIB="$TOOL/lib"
    ...
    "$TOOL/bin/nvcc" -O2 -std=c++17 -arch=sm_86 -ccbin "$HOSTCXX" -I"$TOOL/include" \
      -c "$HERE/rig_philox_race.cu" -o "$HERE/rig_philox_race.o"
    "$HOSTCXX" $COMMON -fopenmp -DM1_USE_CUDA $INC -I"$TOOL/include" "$HERE/$SRC" "$HERE/qwen_cublaslt_hook.cpp" "$HERE/rig_philox_race.o" -o "$HERE/$BIN" \
      $LIBS $CUDALIBS -lm \
      -Wl,-rpath,"$BUILD/src" -Wl,-rpath,"$BUILD/src/ggml-cuda" -Wl,-rpath,"$TOOLLIB" -Wl,-rpath,/usr/lib
  else
    BUILD="$GGML/build-cpu"; LIBS="-L$BUILD/src -lggml -lggml-base -lggml-cpu"
