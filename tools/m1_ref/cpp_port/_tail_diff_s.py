...
for p in pm.primitives: p.material.roughnessFactor=1.0; p.material.metallicFactor=0.0; p.material.metallicRoughnessTexture=None
lo=g.vertices.min(0); hi=g.vertices.max(0)
tgt=np.array([0.35*(hi[0]-lo[0]), lo[1]+0.30*(hi[1]-lo[1]), 0.0])
def R(yaw):
    sc=pyrender.Scene(bg_color=[0.07,0.07,0.08,1],ambient_light=[0.5,0.5,0.5]);sc.add(pm)
    ...
    sc.add(pyrender.DirectionalLight(intensity=1.6),pose=P)
