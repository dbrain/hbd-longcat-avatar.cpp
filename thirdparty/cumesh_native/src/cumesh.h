...
struct HostAtlasCharts {
    int num_charts = 0;
    std::vector<int> chart_ids;
    std::vector<int> chart_vertex_map;
    std::vector<int> chart_faces;          // flat [N*3]
    std::vector<int> chart_vertex_offset;  // [num_charts+1]
    std::vector<int> chart_face_offset;    // [num_charts+1]
};
...
public:
    ...
    void init_host(const float* vertices, size_t num_vertices, const int64_t* faces, size_t num_faces);
    void read_host(std::vector<float>& vertices, std::vector<int64_t>& faces);
   ...
   /**
     ...
     */
    HostAtlasCharts read_atlas_charts_host();
};
