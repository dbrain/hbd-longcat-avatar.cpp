#!/usr/bin/env bash
# Lever L5 prod combo: overlap HtoD weight-stream stall WITHOUT the pool VRAM blowup.
#  off   = baseline (no offload thread)
#  pf    = prefetch thread + pool OFF (the VRAM-safe fix)
#  pfsr  = + shared-resident (pin ~3GB shared params -> less re-stream, LTX got -5.2%)
# 480x832 FR=21 4-step maxv7.3 ov0.25. Measure DiT hi/lo + gen + peak + frame bit-exact vs off.
set -uo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")"
REPO="$PWD"; BUILDER="longcat-avatar-dev:builder"; OUT="$REPO/perf_out/offcombo"; mkdir -p "$OUT/gcache"
...
run(){ local tag=$1; shift; local D="$OUT/$tag"; rm -rf "$D"; mkdir -p "$D"; local t0=$(date +%s.%N)
  docker run --rm --gpus all -v "$REPO:/src" -v "$REPO/models:/models" -w /src \
    -e VACE_GRAY_CACHE_DIR=/src/perf_out/offcombo/gcache "$@" "$BUILDER" \
    ...
    -o /src/perf_out/offcombo/$tag/f%03d.png -v > "$D/run.log" 2>&1 &
  ...
  printf "  %-6s rc=%s  DiT_hi=%-7s DiT_lo=%-7s gen=%-8s peak=%sMiB wall=%.1fs\n" \
    "$tag" "$rc" "${hi:-?}" "${lo:-?}" "${gen:-FAIL}" "$(cat $D/peak.txt 2>/dev/null)" "$(awk "BEGIN{print $(date +%s.%N)-$t0}")"
}
echo "=== offload combo A/B (480x832 FR=21 4-step maxv7.3 ov0.25) ==="
run off
run pf   -e LONGCAT_OFFLOAD_PREFETCH_THREAD=1 -e LONGCAT_NO_PREFETCH_POOL=1 -e LONGCAT_NO_OFFLOAD_PIPELINING=1
run pfsr -e LONGCAT_OFFLOAD_PREFETCH_THREAD=1 -e LONGCAT_NO_PREFETCH_POOL=1 -e LONGCAT_NO_OFFLOAD_PIPELINING=1 -e LONGCAT_SHARED_RESIDENT=1
echo "=== frame bit-exact vs off ==="
for v in pf pfsr; do d=0;n=0; for f in "$OUT"/off/f*.png; do bn=$(basename "$f"); n=$((n+1)); cmp -s "$f" "$OUT/$v/$bn" || d=$((d+1)); done; echo "  $v: $n frames, $d differ"; done
