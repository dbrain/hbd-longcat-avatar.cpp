...
int main(int argc, char** argv) {
    ...
    std::shared_ptr<ModelManager> dit_manager;
    {
        ...
        dit = std::make_shared<IT::InfiniteTalkRunner>(backend, loader.get_tensor_storage_map(), "model.diffusion_model", dit_manager);
        if (!dit_manager->register_runner_params("InfiniteTalk DiT", *dit, "model.diffusion_model",
                                                 ModelManager::ResidencyMode::ParamBackend,
                                                 backend, dit_params_backend) ||
            !dit_manager->validate_registered_tensors()) {
            printf("ERROR: register DiT tensors\n"); return 1;
        }
        dit->set_flash_attention_enabled(getenv("IT_NO_FLASH") == nullptr);
        if (dit_offload) {
            dit->set_stream_layers_enabled(true);
            dit->set_max_graph_vram_bytes((size_t)(max_vram_gib * 1024.0 * 1024.0 * 1024.0));
        }
        printf("DiT registered (%zu tensors)\n", dit_manager->tensor_names().size());
    }
    ...
    if (!w2v_path.empty()) {
        ...
        WAV2VEC2::Wav2Vec2Params w2v_params;
        w2v_params.d_model = 768; w2v_params.n_layers = 12; w2v_params.n_heads = 12;
        w2v_params.head_dim = 64; w2v_params.ffn_dim = 3072; w2v_params.n_hidden_states = 13;
        w2v = std::make_shared<WAV2VEC2::Wav2Vec2EncoderRunner>(audio_backend, loader.get_tensor_storage_map(),
                                                                "audio_encoder", w2v_manager, w2v_params);
        if (!w2v_manager->register_runner_params("InfiniteTalk wav2vec2", *w2v, "audio_encoder",
                                                 ModelManager::ResidencyMode::ParamBackend,
    ...
    if (!vae_path.empty()) {
        ...
        vae = std::make_shared<WAN::WanVAERunner>(backend, loader.get_tensor_storage_map(), "first_stage_model", false, VERSION_WAN2, vae_manager);
        if (!vae_manager->register_runner_params("InfiniteTalk VAE", *vae,
                                                 ModelManager::ResidencyMode::ParamBackend,
                                                 backend, backend) ||
    ...
    std::shared_ptr<ModelManager> clip_manager;
    if (!clip_path.empty()) {
        printf("loading CLIP-H vision '%s'\n", clip_path.c_str());
        clip_manager = std::make_shared<ModelManager>();
        auto& loader = clip_manager->loader();
        ...
        if (!loader.init_from_file_and_convert_name(clip_path, "cond_stage_model.transformer.")) {
            ...
            clip_vision = std::make_shared<FrozenCLIPVisionEmbedder>(backend, loader.get_tensor_storage_map(), clip_manager);
            if (!clip_manager->register_runner_params("InfiniteTalk CLIP-H", *clip_vision,
                                                      ModelManager::ResidencyMode::ParamBackend,
                                                      backend, backend) ||
                !clip_manager->validate_registered_tensors()) {
                printf("ERROR: register CLIP tensors\n"); return 1;
            }
            clip_vision->set_flash_attention_enabled(true);
            printf("CLIP-H registered (%zu tensors)\n", clip_manager->tensor_names().size());
        }
    }
    ...
    auto encode_text = [&](const std::string& text) -> sd::Tensor<float> {
        ...
        auto t5 = std::make_shared<T5Embedder>(audio_backend, loader.get_tensor_storage_map(), "text_encoders.t5xxl.transformer", true, t5_manager);
        if (!t5_manager->register_runner_params("InfiniteTalk umT5", t5->model, "text_encoders.t5xxl.transformer",
                                                ModelManager::ResidencyMode::ParamBackend,
                                                audio_backend, audio_backend) ||
