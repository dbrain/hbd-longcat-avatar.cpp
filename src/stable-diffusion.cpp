...
SD_API bool generate_video_ex(sd_ctx_t* sd_ctx,
    ...
    if (final_latent.empty()) {
        ...
        // Plain (unconditioned) t2v: no continuation/keyframe guide, no char-ref, no driving audio, no
        // v2v/vace. The windowed loop below already handles the no-reference/no-audio case (the audio,
        // guide and char-ref blocks are all conditional and the non-reference RoPE builder is used),
        // so a long plain t2v can tile for VRAM exactly like the conditioned paths instead of forcing a
        // single full-length pass whose peak grows unbounded with frame count. For a segment that still
        // fits one window this is a no-op (single full pass below), so all current short renders stay
        // byte-identical; only clips longer than the window gain the windowed path.
        const bool base_window_plain =
            latents.video_positions.empty() && latents.video_reference.empty() &&
            !latents.audio_fixed && latents.audio_length == 0 &&
            sd_vid_gen_params->control_frames_size == 0 && sd_vid_gen_params->v2v_mode == 0;
        const bool base_temporal_windowing = base_window_env != nullptr && base_window_env[0] != '\0' &&
                                             std::string(base_window_env) != "0" &&
                                             sd_version_is_ltxav(sd_ctx->sd->version) &&
                                             (base_window_legacy_audio || base_window_ref || base_window_plain) &&
        ...
        if (!base_temporal_windowing) {
            ...
            if (base_window_ref && (base_target_frames <= temporal_window || !base_window_ref_layout_ok)) {
                ...
                final_latent = sample_base_window(x_t,
                                                  ...
                                                  latents.audio_positions,
                                                  latents.video_reference);
            } else if (base_window_plain && base_target_frames <= temporal_window) {
                // Plain t2v that already fits one window: full pass, byte-identical to the historical
                // non-windowed path — no tiling overhead when it isn't needed.
                final_latent = sample_base_window(x_t,
                                                  std::move(noise),
                                                  latents.denoise_mask,
                                                  latents.audio_length,
                                                  ...
                                                  latents.audio_positions,
                                                  latents.video_reference);
            } else if (!base_window_has_audio && !base_window_ref && !base_window_plain) {
