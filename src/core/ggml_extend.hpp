...
__STATIC_INLINE__ ggml_tensor* ggml_ext_conv_3d(ggml_context* ctx,
    ...
    if (tile_w || tile_h) {
        ...
        do {
            int64_t oh1;
            ggml_tensor* xh;
            // cuDNN Conv3D may not retain arbitrary strided views across the tiled VAE graph ->
            // a non-contiguous tile slice decodes/encodes as an ~8px latent grid (GPU-proven on v2v
            // SDEdit at partial strength; tile-count-dependent). Materialize each tile so VAE encodes
            // (i2v/v2v/retake anchors) and hi-res decodes get contiguous input.
...
__STATIC_INLINE__ ggml_tensor* ggml_ext_attention_ext(ggml_context* ctx,
    ...
    if (flash_attn) {
        ...
        // (upstream #1453 removed kv_pad unconditionally; we keep it opt-out so the
        //  legacy path stays available for callers that still rely on it.)
        ...
        for (const auto& input : segment.input_refs) {
            ...
            switch (input.type) {
                ...
                case GraphCutSegment::INPUT_EXTERNAL: {
                    // LongCat lap-27: skip runner-registered persistent backend leaves
                    // (e.g. the avatar's cond-K/V cache tensors). They have a valid
                    // non-compute backend buffer (registered via register_persistent_tensor
                    // in the owning runner) and must retain their buffer/data/extra so the
                    // per-segment gallocr leaves them alone and the segment reads the right
                    // data. (Coexists with upstream's per-call persistent_externals map.)
...
public:
    ...
    bool alloc_params_buffer() {
        ...
        // (Equivalent to upstream #1601's generic dev_host_buffer_type path; kept the
        //  fork's CUDA-specific variant for the env opt-out + explicit fallback log.)
