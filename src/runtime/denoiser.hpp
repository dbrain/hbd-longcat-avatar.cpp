...
static sd::Tensor<float> sample_euler_ancestral_cfg_pp(denoise_cb_t model,
    ...
    for (int i = 0; i < steps; i++) {
        ...
        sd::Tensor<float> denoised        = std::move(denoised_opt.pred);          // cfg-combined x0 (=cond x0 at cfg1)
        ...
        if (is_flow_denoiser) {
            ...
            // ancestral step there, then rescales by alpha_t. Using raw flow sigmas (the old
            // code) over-guides ~2.5x and over-weights the denoised anchor.
            ...
            if (alpha_s < 1e-6f) alpha_s = 1e-6f;   // guard sigma->1 endpoint (comfy offset_first_sigma_for_snr)
            ...
            sd::Tensor<float> d = (x - uncond_denoised * alpha_s) / sigma;  // direction from uncond, VP-normalized
