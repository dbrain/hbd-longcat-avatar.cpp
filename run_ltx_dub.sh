...
#   SRC=cont_seg1  ./run_ltx_dub.sh      # SRC = a dir of Wan2.2 frames (8k+1 count)
set -uo pipefail
...
if [ -f "$OUT/relip.webm" ]; then
  ffmpeg -y -i "$OUT/relip.webm" -c:v libx264 -crf 18 -pix_fmt yuv420p -c:a aac \
    "$REPO/tools/lightx2v-test/out/ltx_dub_$TAG.mp4" -loglevel error 2>/dev/null && echo "muxed (model-GENERATED audio) -> ltx_dub_$TAG.mp4"
fi
