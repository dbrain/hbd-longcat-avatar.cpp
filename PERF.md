...
| 24b | **conv-3d-direct WMMA prototype — MEASURED 8.5× SLOWER, NOT SHIPPED (default-off behind `LONGCAT_CONV3D_DIRECT`).** Lever 2 attempt: a tensor-core implicit-GEMM for `GGML_OP_CONV_3D` (CPU-only before) that gathers the im2col B-tile on-the-fly instead of materializing the 27×IC expansion, adapted from qwen3-tts `conv-1d-direct.cu` via the index mapping reverse-engineered from `ggml_compute_forward_conv_3d_impl`. **Correct** (25f --steps-1: PSNR ~40 dB / ac16 0.844 ≡ baseline — coherent; ~40 not 99 is the F16×F16→F32-accum precision shift, not a bug) **but VAE decode 15.09 → 128.22 s (8.5× slower).** Root suspect (not yet micro-profiled): naive BM=16 tiling **re-gathers B once per oc-tile** (oc=96–384 ⇒ 6–24× redundant execution of the expensive 6-int-div/element gather), vs im2col+cuBLAS materializing B ONCE with cache reuse; tiny 16×64 WMMA tiles = low arithmetic intensity. **Verdict: lever 2 is fork-class GEMM engineering (12–20h: large register/warp tiles so one block covers all oc → B gathered once per patch-tile, smem B-reuse, K-tiling), NOT a port — re-derived the lap-12 "naive port backfires" warning the hard way.** Prototype kept default-off as the correct foundation; default path emits no CONV_3D node (fully inert). **Measure-don't-predict win: the op-profile's "fuse im2col = ~33%" was a ceiling, not a forecast.** Next-decision: grind lever-2 GEMM vs pivot to lever 3 (DiT, 77% of wall). | 25f VAE decode unchanged (prototype default-off) | n/a | correct/coherent (PSNR 40 / ac16 0.844), not perf-viable | ggml `88270c05` (`conv-3d-direct.cu`) + parent (this lap) |
| 24  | **CONT pixel-shuffle + DiT-attn permute copies coalesced (VAE −14.7%, DiT −1.7%, SHIPPED, bit-exact).** lap-23's re-profile pinned CONT as the #1 VAE op (~4s, 22.8%): `ggml_cont(permute(...))` falls through to `cpy_scalar`, which decomposes the flat index with **4 int64 divisions PER element** and (for the pixel-shuffle shapes) reads uncoalesced. Measured the 4310 ms of strided SCALAR cont: **3.14s "transposable"** (one src axis off dim0 carries the unit stride → dim0 reads stride-jump) + **1.17s "f0-coalesced"** (dim0 already unit-stride, but ALU-bound on the per-element divide). Two new F32 kernels in `cpy.cu`, dispatched for cont-to-contiguous: **(a) `cpy_perm_transpose`** — smem-tiled 2D transpose between dst-axis0 and the unit-stride axis f, batched over the other two axes with REAL strides (the stock `cpy_scalar_transpose` only does f==1 + ne[3]==1 + dense batch); coalesces both read (along f) and write (along 0). **53–122 → 328–331 GB/s (memcpy roofline, up to 6.2×).** **(b) `cpy_perm_coalesced`** — dim0 already unit-stride both sides, higher dims permuted; each block owns one (i1,i2,i3), decomposes the batch index ONCE, streams dim0 contiguously (kills the per-element divide). **147–221 → 315–468 GB/s (2.2× on the DiT `[128,10920,32,1]` attn permute).** grid.z batch guarded <65536. **Pure element copies ⇒ bit-exact by construction:** `LONGCAT_CONT_VERIFY` byte-compares vs the reference scalar path (6617 conts, **0 mismatches**) + --steps-1 AND --steps-8 render **PSNR 99.00 dB / byte-identical webm** (ac16 0.832–0.834 flat, no melt). **Measured (25f): VAE decode 17.70→15.10 s (−14.7%, two runs ±0.02); DiT sampling 139.9→137.5 s (−1.7%, the attn permutes ×8 steps — lever 1b helps the DiT too); cont-op serialized total ~4.9→~1.85 s (−62%).** RE-PROFILE: CONT drops #1→#5 (22.8%→8.9%); new VAE order MUL_MAT 26.4% (cuBLAS floor) / IM2COL_3D 21.8% / CONCAT 11.4% / IM2COL-2D 11.3% → **next = lever 2 (conv-3d-direct WMMA fuses away both IM2COLs = 33%).** Env: `LONGCAT_CONT_NOTRANSP` (force old path), `LONGCAT_CONT_VERIFY`; CONT_PROF gains PERMT/COAL labels. | 25f VAE decode 17.70→**15.10 s** | unchanged ~5.7 GB | **bit-exact (PSNR 99.00 / VERIFY 0/6617); 8-step coherent ac16 0.83** | ggml `740156c2` (`cpy.cu`) + parent (this lap) |
| 23  | **im2col_3d smem-halo tiled kernel + half2 vectorized F16 store (VAE −39%, SHIPPED, bit-exact).** lap-22's fastdiv kernel was gather-read-latency bound (51 GB/s = 14% peak): each output tap did an independent cache-line-wasteful global load of its 3×3×3×ICh neighborhood. Fix: a smem-halo tiled fast path for the Wan-VAE conv shapes (stride=1/dil=1/pad=0 — CausalConv3d pre-pads externally so im2col sees pad=0 ⇒ OD=ID−KD+1 etc. ⇒ **every tap in-bounds, no boundary checks**). A block stages the reused input window for a (channel-block CB × all-OD × TOH×TOW) output tile into shared memory ONCE (coalesced), threads write columns from smem (input re-read ~1.4× halo vs ~10× cache-line amplification). On top, **VEC=2 packs the two consecutive F16 column entries into one 32-bit `half2` store** (per-warp write 64B→128B, the lever past the ~155 GB/s scalar-store plateau). Default tile 8,8,8,4 (sm_86 sweep). fastdiv retained as universal fallback (`im2col_3d_tiled_try` returns false on non-unit stride/dil, non-zero pad, smem>48KB). **Measured (25f --steps-1): im2col_3d total 15367→3701 ms (−76%, 4.15×); VAE decode 29.2→17.7 s (−39%); big call (OD=4 OH=256, 1296 MiB) 27→5.68 ms (50→239 GB/s, 66% peak).** Bit-exact: `LONGCAT_IM2COL_VERIFY` (re-runs fastdiv into temp + byte-compares, 0/800+ mismatches) + --steps-1 tiled-vs-forced-fastdiv PSNR **99.00 dB**. 8-step clip coherent (ac16 0.831–0.840 flat, no end-of-clip melt). **DEAD-END measured: VEC=8 (uint4 128-bit) a wash vs half2** (1885–1906 vs 1906 ms big call; smaller blockDim.x + bigger smem cancel the wider store). **RE-PROFILE (serialized≈real, 17.2 vs 17.7s): the VAE is no longer im2col-bound — CONT 22.8% (~4s, pixel-shuffle permute-copies, NEW #1), MUL_MAT 22.4% (~4s, cuBLAS floor), IM2COL_3D 18.4%, IM2COL-2D 9.6%, CONCAT 9.6%.** Next levers: CONT fused-shuffle (cpy_scalar coalescing or fused depth↔space) + conv-3d-direct WMMA (must stay tensor-core or it regresses MUL_MAT). | 25f VAE decode 29.2→**17.7 s** (im2col 15.4→3.7 s) | unchanged ~5.7 GB | **bit-exact (PSNR 99.00 dB / VERIFY 0 mismatch); 8-step coherent ac16 0.83–0.84** | ggml `9966677a` (`im2col.cu`) + parent `03ba865`, this lap |
